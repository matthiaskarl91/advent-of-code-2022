...
fn section_overlapping(range: Range<u32>, section: Vec<u32>) -> bool {
    if range.contains(&section[0]) || range.contains(&section[1]) {
        return true;
    }
    false
...
pub async fn day_four() -> Result<(), Error> {
    ...
    let parsed_assignments: Vec<Vec<Vec<u32>>> = res
        .split('\n')
    ...
    let part_two: u32 = parsed_assignments
        ...
        .map(|sections| {
            ...
            if sections.len() > 1 {
                ...
                if section_overlapping(range1, second_section)
                    || section_overlapping(range2, first_section)
                {
                    sum_overlapping += 1;
                }
            }
        ...
        .sum();
    println!("Day 4: {}", part_one);
    println!("Day 4: {}", part_two);
    Ok(())
}
